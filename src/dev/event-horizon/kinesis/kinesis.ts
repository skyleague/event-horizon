...
>(
    ...
    const payload = (
        kinesis.schema.payload !== undefined ? arbitrary(kinesis.schema.payload as Schema<unknown>) : json()
    ...
    return tuple(record, payload).map(([r, p]) => {
        ...
        event.raw.kinesis.data = Buffer.from(JSON.stringify(event.payload)).toString('base64')
